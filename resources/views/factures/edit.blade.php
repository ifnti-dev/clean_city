<x-app-layout>
    ...
    <x-slot>
        ...
        <div class=" card  mt-[-50px] p-5 mx-4 mb-6 ">
            <form method="POST" action="{{ route('factures.update', $facture->id) }}" class="formulaire">
                @csrf
                @method('PUT')
                        ...
                        <div class="mt-4 w-full">
                            ...
                            <select name="tarif_id" id="tarif_id" placholder="" class="w-full rounded-lg">
                                @foreach ($tarifs as $tarif)
                                    <option  @selected(old('tarif_id') == $tarif->id) value="{{ old('tarif_id', $tarif->id) }}"  data-montant="{{ $tarif->montant }}">
                            ...
                            <div class="mt-4 w-full">
                                ...
                                <select name="abonnement_id" id="abonnement_id" placholder="" class="w-full rounded-lg">
                                    @foreach ($abonnements as $abonnement)
                                        <option @selected(old('abonnement_id') == $abonnement->id) value="{{ old('abonnement_id', $abonnement->id) }}">
                        ...
                        <div class="mt-4 w-full">
                            ...
                            <select name="methode_paiement_id" id="methode_paiement_id" placholder=""
                                ...
                                @foreach ($methode_paiements as $methode_paiement)
                                    <option @selected(old('methode_paiement_id') == $methode_paiement->id) value="{{ old('$methode_paiement', $methode_paiement->id) }}">
